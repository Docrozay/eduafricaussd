...
// Reads the variables sent via POST
    ...
    $check_phone2=$con->prepare("SELECT * FROM ken_stu_ussd WHERE stu_phone=? AND stu_status=?");
    $check_phone2->setFetchMode(PDO:: FETCH_ASSOC);
    ...
    if($check_phone2->execute([$phone,"YES"])){
        $user_count2=$check2_phone->rowCount(); 
        if($student = $check2_phone->fetch()){
            $student_firstname = $student['stu_firstname'];
            $student_lastname = $student['stu_lastname'];
            $student_lga = $student['stu_lga'];
            $student_status = $student['stu_status'];
